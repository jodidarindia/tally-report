  ...
  return (
    <div data-testid="history-page">
      <div className="mb-8">
        ...
        <p className="mt-2 text-base text-slate-600">View your past AI-generated reports</p>
      </div>
      ...
      <div className="space-y-4">
        {history.length > 0 ? (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-lg font-medium text-slate-900">{item.query_text}</h3>
                  </div>
                  {item.created_at && (
                    <div className="flex items-center gap-2 text-sm text-slate-500 mb-3">
                      <Clock size={14} />
                      {(() => { const r = item.created_at; const d = (r && (r.includes('+') || r.includes('Z'))) ? new Date(r) : new Date(r + 'Z'); return d.toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }); })()}
                    </div>
                  )}
                  {item.response && (
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-12 text-center">
            ...
            <p className="text-slate-500">Start using the AI Query Builder to generate reports</p>
          </div>
        )}
      </div>
    </div>
